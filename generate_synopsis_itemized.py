"""EchoSphere Project Synopsis PDF Generator (Itemized Tools Variant).

Generates a single-page EchoSphere Project Synopsis PDF strictly using
TrueType 'Times New Roman' throughout the entire document, with large, prominent
academic typography matching the final PDF, an expanded signature area, and staff
designations/names positioned at the absolute bottom of the page with zero overflow.
"""
...
def generate_synopsis_itemized_pdf(output_filename="EchoSphere_Synopsis_Itemized.pdf"):
    """Build and save the single-page EchoSphere synopsis PDF with large typography and bottom signatures."""
    ...
    # --- Strict Times New Roman Typography Styles (Prominent, Large & Clean) ---
    ...
    tool_item_style = ParagraphStyle(
        'ToolItemCustom',
        parent=styles['Normal'],
        fontName=base_font,
        fontSize=9.2,
    ...
    # 6. LANGUAGE / SOFTWARE TOOLS USED (LINE BY LINE)
    story.append(Paragraph("<b>Language / Software Tools used:</b>", section_heading_style))
    story.append(Spacer(1, 1))
    tools_lines = [
        "&bull; <b>OS &amp; Clients:</b> Windows (Desktop), Android (Mobile / Tablet)",
        "&bull; <b>Languages:</b> Python 3.11+, Dart 3.x",
        "&bull; <b>Frameworks:</b> Flutter 3.x, FastAPI (Uvicorn, Async SQLAlchemy, Pydantic, Alembic)",
        "&bull; <b>Database &amp; Cache:</b> PostgreSQL 15, Redis",
        "&bull; <b>AI/ML &amp; Voice Stack:</b> Google Gemini 1.5 API, Whisper STT, PyTorch, Neural TTS",
        "&bull; <b>Hardware Module (Planned):</b> ESP32-S3 / Raspberry Pi Zero 2 W, PCM5102A I2S DAC, TPA3116D2 Amplifier, 5V Relay, PA Speakers",
        "&bull; <b>APIs &amp; Protocols:</b> REST API, WebSockets, MQTT, FCM",
    ]
    for t_line in tools_lines:
        story.append(Paragraph(t_line, tool_item_style))
    ...
    # Large signature space (48 pt) pushing staff block flush to the absolute bottom of the page
...
if __name__ == "__main__":
    generate_synopsis_itemized_pdf()
