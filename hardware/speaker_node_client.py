...
class SpeakerNodeClient:
    ...
    def play_audio(self, audio_url: str, title: str, message: str = "", is_emergency: bool = False):
        ...
        if audio_url:
            ...
            tmp_filename = f"tmp_announcement_{uuid.uuid4().hex[:8]}{ext}"
        ...
        # 3. Fallback: Speak the text directly via local neural voice synthesis (Kokoro-grade)
        if not played:
            text_to_speak = f"{title}. {message}" if message else title
            logger.info(f"🎙️ [KOKORO NEURAL VOICE] Synthesizing speech aloud: '{text_to_speak}'")
    ...
    def _run_speaker_test(self):
        self.current_status = "PLAYING"
        play_chime()
        test_msg = "EchoSphere smart speaker Node 1 diagnostic test passed. Audio subsystem is operational with Kokoro neural voice."
