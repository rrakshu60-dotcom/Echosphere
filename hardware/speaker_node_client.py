...
class SpeakerNode1Client:
    ...
    def play_announcement_sync(
        ...
        with self._playback_lock:
            ...
            if full_audio_url and full_audio_url.startswith("http"):
                ...
                tmp_file = os.path.join(os.path.dirname(__file__), f"node1_play_{uuid.uuid4().hex[:6]}{ext}")
                try:
    ...
    def run_diagnostic_test(self):
        ...
        with self._playback_lock:
            ...
            play_attention_chime()
            test_msg = f"EchoSphere Smart Speaker Node 1 diagnostic self test. Audio subsystem is operational in {self.zone}. Volume is at {self.volume} percent."
